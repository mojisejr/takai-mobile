...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), Array.from({ length: 18 }, (_, index) => index + 1), 'fresh schema must include additive advance-recovery, payment-session, and V2 foundation ledgers');
