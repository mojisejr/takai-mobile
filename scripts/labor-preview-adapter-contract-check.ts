...
import { LABOR_PREVIEW_WEB_PROJECTIONS, LABOR_PREVIEW_WEB_READ_MODEL } from '../src/features/labor-mvp/preview.web.fixture';
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(LABOR_PREVIEW_WEB_PROJECTIONS.today, await getLaborTodaySummary(db, '2026-08-02'), 'web today projection must be generated by the repository');
    assert.deepEqual(LABOR_PREVIEW_WEB_PROJECTIONS.calendar, await getLaborCalendarRange(db, { startDate: '2026-08-01', endDate: '2026-08-31' }), 'web calendar projection must be generated by the repository');
    assert.deepEqual(LABOR_PREVIEW_WEB_PROJECTIONS.history, await getLaborHistory(db, { startDate: '2026-08-01', endDate: '2026-08-31' }), 'web history projection must be generated by the repository');
    assert.deepEqual(LABOR_PREVIEW_WEB_PROJECTIONS.jobs['labor-preview-group-piece'], await getLaborJobDetail(db, 'labor-preview-group-piece'), 'web job detail must retain group settlement meaning');
    assert.deepEqual(LABOR_PREVIEW_WEB_PROJECTIONS.people['labor-preview-chon'], await getLaborPersonDetail(db, 'labor-preview-chon'), 'web person detail must retain advance meaning');
