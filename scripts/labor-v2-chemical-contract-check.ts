...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), Array.from({ length: 21 }, (_, index) => index + 1), 'fresh notebook reaches chemical migration 21');
    ...
    assert.deepEqual(await runMigrations(upgradeDb), [20, 21], 'migration-19 notebook upgrades through additive chemical library and task-mix migrations'); upgrade.close();
