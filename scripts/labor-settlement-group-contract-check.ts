...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), [12, 13, 14, 15, 16], 'schema-11 upgrade must apply only additive settlement-group, work-basis, hourly-duration, advance-recovery, and payment-session migrations');
