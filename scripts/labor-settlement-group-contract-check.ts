...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), [12, 13, 14, 15, 16, 17, 18, 19, 20], 'schema-11 upgrade must apply only additive settlement-group, work-basis, hourly-duration, advance-recovery, payment-session, V2 foundations, plot context, and chemical library');
