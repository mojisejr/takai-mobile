...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), [13, 14, 15, 16, 17, 18, 19, 20, 21], 'schema-12 upgrade must add immutable work-basis, hourly-duration, advance-recovery, payment-session, V2 foundations, plot context, chemical library, and task mixes');
