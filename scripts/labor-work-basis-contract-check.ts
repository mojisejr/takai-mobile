...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), [13, 14, 15, 16, 17, 18], 'schema-12 upgrade must add immutable work-basis, hourly-duration, advance-recovery, payment-session, and V2 foundations');
