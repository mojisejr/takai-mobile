...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), Array.from({ length: 18 }, (_, index) => index + 1), 'fresh database must include only additive V2 foundation migrations');
    ...
    assert.deepEqual(v2Tables.map((table) => table.name), ['labor_v2_contract_batch_members', 'labor_v2_contract_batch_task_links', 'labor_v2_contract_batches', 'labor_v2_contract_progress', 'labor_v2_daily_unit_task_links', 'labor_v2_daily_units', 'labor_v2_event_history', 'labor_v2_hourly_shifts', 'labor_v2_hourly_time_entries', 'labor_v2_obligations', 'labor_v2_payment_advance_recoveries', 'labor_v2_payment_allocations', 'labor_v2_payment_recipient_settlements', 'labor_v2_payment_revisions', 'labor_v2_payment_sessions', 'labor_v2_task_assignments', 'labor_v2_work_tasks'], 'V2 must own distinct work, unit, contract, obligation, payment, and history tables');
