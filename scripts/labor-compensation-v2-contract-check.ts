...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), Array.from({ length: 19 }, (_, index) => index + 1), 'fresh database must include additive V2 foundation and plot migrations');
