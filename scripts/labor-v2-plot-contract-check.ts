...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    assert.deepEqual(await runMigrations(db), Array.from({ length: 19 }, (_, index) => index + 1), 'fresh database reaches the additive V2 plot migration');
    ...
    assert.deepEqual(await runMigrations(upgradeDb), [19], 'migration-18 notebooks upgrade through one additive plot migration'); upgrade.close();
