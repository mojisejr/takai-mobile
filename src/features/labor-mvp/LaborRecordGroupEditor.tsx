...
type Job = { id: string; title: string; minutes: string };
...
const createJob = (): Job => ({ id: `job-${Date.now()}-${Math.random().toString(36).slice(2)}`, title: '', minutes: '' });
...
export function LaborRecordGroupEditor({ ready, refresh, notify, onPayment }: Props) {
  ...
  const [groups, setGroups] = useState<WorkGroup[]>(() => {
    if (scenario === 'daily-three-task') return [{ ...createGroup('daily'), id: 'proof-daily', personId: su, rateBaht: '350', jobs: [{ id: 'proof-cut', title: 'ตัดหญ้า', minutes: '' }, { id: 'proof-fertilize', title: 'ใส่ปุ๋ย', minutes: '' }, { id: 'proof-spray', title: 'พ่นยา', minutes: '' }] }];
    if (scenario === 'open-contract') return [{ ...createGroup('contract'), id: 'proof-contract', memberIds: [su, phuang], contractTitle: 'กรอกถุงเพาะชำ', jobs: [{ id: 'proof-bags', title: 'กรอกถุงเพาะชำ', minutes: '' }] }];
    return [createGroup()];
  });
  ...
  const save = async () => {
    ...
    for (const group of groups) {
      ...
      if (group.method === 'hourly' && (!Number.isSafeInteger(bahtToSatang(group.rateBaht)) || bahtToSatang(group.rateBaht) <= 0 || group.jobs.some((job) => !Number.isSafeInteger(Number(job.minutes)) || Number(job.minutes) <= 0))) return notify('กรอกค่าแรงรายชั่วโมงและเวลาของแต่ละงานให้ครบ');
    }
    ...
    const hourly = groups.filter((group) => group.method === 'hourly').flatMap((group) => group.jobs.filter((job) => job.title.trim()).map((job) => ({ id: `${group.id}:${job.id}`, taskId: `${group.id}:${job.id}`, personId: group.personId!, rateSatang: bahtToSatang(group.rateBaht), shiftKey: group.id, durationMinutes: Number(job.minutes) })));
    setSaving(true);
    try {
  ...
  return <View style={styles.screen}>
    ...
    {groups.map((group, index) => <FieldCard key={group.id} variant="raised">
      ...
      {group.jobs.map((job, jobIndex) => <View key={job.id} style={styles.jobRow}><TextInput accessibilityLabel={`งานที่ทำวันนี้ ${jobIndex + 1}`} onChangeText={(title) => updateJob(group.id, job.id, { title })} placeholder={`งานที่ ${jobIndex + 1}`} style={styles.input} value={job.title} />{group.method === 'hourly' ? <TextInput accessibilityLabel={`เวลางาน ${jobIndex + 1}`} keyboardType="number-pad" onChangeText={(minutes) => updateJob(group.id, job.id, { minutes })} placeholder="นาที" style={[styles.input, styles.minutes]} value={job.minutes} /> : null}<Pressable accessibilityLabel={`ลบงานที่ ${jobIndex + 1}`} onPress={() => removeJob(group.id, job.id)}><Text style={styles.remove}>ลบ</Text></Pressable></View>)}
