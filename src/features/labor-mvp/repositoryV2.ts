...
type ChemicalUseHistoryRow = TaskChemicalUseRow & { work_date: string; title: string; water_litres: number };
...
export const getLaborV2ChemicalDetail = async (db: SqlExecutor, chemicalId: string): Promise<LaborV2ChemicalDetail> => {
  ...
  const usages = await db.getAllAsync<ChemicalUseHistoryRow>('SELECT task.id AS task_id, task.work_date, task.title, mix.water_litres, use_row.chemical_id, use_row.common_name_snapshot, use_row.reference_amount_snapshot, use_row.reference_unit_snapshot, use_row.reference_water_litres_snapshot, use_row.calculated_amount, use_row.marked_empty, use_row.sort_order FROM labor_v2_task_chemical_uses use_row JOIN labor_v2_task_chemical_mixes mix ON mix.id = use_row.task_chemical_mix_id JOIN labor_v2_work_tasks task ON task.id = mix.task_id WHERE use_row.chemical_id = ? ORDER BY task.work_date DESC, task.id DESC, use_row.sort_order ASC, use_row.id ASC', [chemicalId]);
  const usageSnapshots: LaborV2ChemicalUseHistory[] = usages.map((item) => ({ taskId: item.task_id, workDate: item.work_date, taskTitle: item.title, waterLitres: Number(item.water_litres), commonName: item.common_name_snapshot, referenceAmount: Number(item.reference_amount_snapshot), referenceUnit: item.reference_unit_snapshot, referenceWaterLitres: Number(item.reference_water_litres_snapshot), calculatedAmount: Number(item.calculated_amount), wasMarkedEmpty: Boolean(item.marked_empty) }));
  return { ...chemicalSnapshot(row), revisions: revisions.map((item) => ({ id: item.id, chemicalId: item.chemical_id, revision: Number(item.revision), action: item.action, reason: item.reason, before: item.before_json ? JSON.parse(item.before_json) : null, after: JSON.parse(item.after_json), createdAt: item.created_at })), lastUsedOn: usageSnapshots[0]?.workDate ?? null, usages: usageSnapshots };
};
