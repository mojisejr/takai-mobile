...
const adapter = (model: LaborV2ReadModel, mode: 'notebook' | 'proof'): LaborV2Adapter => ({
  ...
  workers: { list: async (includeArchived = false) => mode === 'proof' ? structuredClone(fixtureWorkers.filter((item) => includeArchived || !item.archivedAt)) : [], create: readonly, update: readonly, archive: readonly }, commands: { recordDay: readonly, startContract: readonly, progressContract: readonly, finalizeContract: readonly, postPayment: readonly, correctPayment: readonly },
});
